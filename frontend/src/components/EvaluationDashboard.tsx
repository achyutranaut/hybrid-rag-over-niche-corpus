...
const SparkBar: React.FC<{ value: number; max?: number; highlight?: boolean }> = ({
  ...
  return (
    <div className="inline-flex items-center space-x-2 font-mono text-xs">
      <div className="w-14 h-2 bg-surface-3 border border-border rounded-xs overflow-hidden relative">
        <div
          className={`h-full transition-all duration-300 ${
            highlight ? 'bg-attack shadow-glow-sm' : 'bg-ink-muted/80'
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
      <span className={`font-tabular ${highlight ? 'text-attack font-bold' : 'text-ink-primary'}`}>
        {formatScore(value, 3)}
      </span>
    </div>
  );
...
export const EvaluationDashboard: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-28 text-ink-muted space-y-3 font-mono text-xs">
        <div className="w-5 h-5 border-2 border-attack border-t-transparent animate-spin rounded-full shadow-glow-sm" />
        <span className="text-ink-primary">Loading frozen benchmark evaluation artifacts...</span>
        <span className="text-ink-faint text-[10px]">sha256:cdcc7258098ffa61</span>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="p-4 border border-red-800/60 bg-red-950/20 text-red-300 font-mono text-xs rounded-sm">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Split Selector & Protocol Controller */}
      <Panel
        header={
          <div className="flex flex-col sm:flex-row sm:items-center justify-between w-full gap-3">
            ...
            <div className="flex items-center space-x-1.5 p-1 bg-surface-2 border border-border rounded-sm">
              <button
                type="button"
                onClick={() => setActiveSplit('dev')}
                className={`px-3 py-1.5 text-xs font-mono transition-colors rounded-sm cursor-pointer ${
                  activeSplit === 'dev'
                    ? 'bg-surface-3 text-ink-primary border border-border-strong font-bold shadow-sm'
      ...
      >
        ...
        <div className="flex space-x-2 border-b border-border pb-2 overflow-x-auto text-xs font-mono no-scrollbar">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 whitespace-nowrap transition-colors border-b-2 rounded-t-sm cursor-pointer ${
                activeExp === tab.id
                  ? 'border-attack text-ink-primary font-bold bg-surface-2'
          ...
          <div className="space-y-6 pt-4">
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-border pb-2">
                <div>
                  <h4 className="font-serif font-semibold text-sm text-ink-primary">
                  ...
                  <tbody className="font-mono text-xs">
                    {Object.entries(activeSplit === 'test' ? b1.test : b1.dev).map(
                      ([modelName, metrics]: [string, MetricSet]) => {
                        ...
                        return (
                          <tr key={modelName} className={isHybrid ? 'bg-surface-2/70 font-semibold' : undefined}>
                ...
                <table className="research-table">
                  ...
                  <tbody className="text-xs">
                    ...
                    <tr>
                      ...
                      <td className="text-ink-muted font-sans">
                        <strong className="text-valid">Hybrid matches Sparse</strong>, bridging CVE records and MITRE ATT&amp;CK techniques.
                      </td>
                    </tr>
                    <tr>
        ...
        {activeExp === 'b2' && b2 && (
          <div className="space-y-6 pt-4">
                  ...
                  <tbody className="font-mono text-xs">
                    {Object.entries(activeSplit === 'test' ? b2.test : b2.dev).map(
                      ([cfgKey, metrics]: [string, MetricSet]) => {
                        ...
                        return (
                          <tr key={cfgKey} className={isCe ? 'bg-surface-2/60 font-semibold' : undefined}>
            ...
            {/* OOD Breakdown */}
            <div className="p-4 bg-surface-2 border border-border rounded-sm space-y-2">
              <h5 className="font-serif font-semibold text-sm text-cve-warn">
                Expanded 70-Query Out-of-Domain Calibration (DEV n=50, TEST n=20)
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono pt-1">
                <div className="p-3 bg-surface-1 border border-border rounded-sm space-y-1">
                  <div className="text-ink-faint">Synthetic OOD (8 queries):</div>
                  <div className="text-base font-bold text-ink-primary">87.5% DEV / 50.0% TEST</div>
                  <div className="text-[11px] text-ink-muted">Uncalibrated synthetic non-security questions</div>
                </div>
                <div className="p-3 bg-surface-1 border border-border rounded-sm space-y-1">
                  <div className="text-ink-faint">Realistic OOD Benchmark (70 queries):</div>
                ...
                <tbody className="font-mono text-xs">
                  {Object.entries(data.runs.e3_floor.summary).map(([arm, m]: [string, any]) => (
                    <tr key={arm}>
                      ...
                      <td><SparkBar value={m.mrr} /></td>
                      <td><SparkBar value={m['ndcg@5']} /></td>
                      <td className="text-valid font-bold">{formatPercent(m.abstention_accuracy)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
